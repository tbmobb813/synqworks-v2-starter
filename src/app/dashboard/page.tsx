...
import type {
  DashboardData, Skill, UserProgress, SkillBreakdownItem, RiskSkill,
  RecentActivityItem, KPIStats, CertificationProgress, AssessmentHistoryPoint, MandatoryTraining
} from '@/types'
...
async function getDashboardData(): Promise<DashboardData | null> {
  try {
    ...
    const certifications: CertificationProgress[] = certs.map((cert: { id: string; name: string; description: string | null; badge_color: string; threshold: number; skill_ids: string[] }) => {
      ...
      return {
        id: cert.id, name: cert.name, description: cert.description, badge_color: cert.badge_color,
        threshold: cert.threshold, total_skills: skillIds.length, skills_met: skillsMet,
        percent_complete: percentComplete, skill_scores: skillScores,
      }
    })
    ...
    const mandatory_trainings: MandatoryTraining[] = mandatoryModules.map((m: { id: string; skill_id: string; title: string; difficulty: number; estimated_mins: number; is_mandatory: boolean; due_date: string | null }) => {
      ...
      return {
        module_id: m.id, title: m.title, skill_name: skill?.name ?? 'Unknown',
        difficulty: m.difficulty, estimated_mins: m.estimated_mins, due_date: m.due_date,
        is_completed: isCompleted, days_remaining: daysRemaining, is_overdue: isOverdue,
      }
    })
